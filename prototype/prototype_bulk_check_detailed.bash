...
check_dir=/cm/shared/workstation_code_dev/archive/specimen_status/prototype/
nlsam_checker=/cm/shared/workstation_code_dev/shared/pipeline_utilities/prototype/prototype_nlsam_check_finished.bash
#these scripts all take a simpler runno as the input
diff_checks=(
    $check_dir/prototype_check_co_reg.bash
    $check_dir/prototype_check_nii4d.bash
    $check_dir/prototype_check_nlsam_mask.bash)
# need new nlnsam checker which fits in this scheme
#    $nlsam_checker)
...
do
    ...
    if [ $log_search_fail -eq 0 ];then
        recon_status=$(echo "$log_line" |sed -r 's/.*TOTAL progress:(.*)/\1/')
        echo "--- $r recon: $recon_status"
###        echo "$log_line";
    ...
    if [ "$diff" != "" ];then
        ...
        then
            ...
            # a simple blank if not ready
            #echo -n "--- "
            # a "complicated" fraction of parts
            d=0;
            t=0;
            for c in ${diff_checks[@]}; do
                let t=$t+1;
                if bash $c $r;
                then let d=d+1;
                fi
